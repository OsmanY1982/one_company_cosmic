@echo off
title Iqra-Hermes 同步安装器 v1.0
chcp 65001 >nul

echo.
echo 🚀 Iqra-Hermes 三地同步安装器 v1.0
echo =====================================
echo.

:: 检查权威源
if not exist "D:\\one_company_desktop\\iqra\\openclaw_adapter.py" (
    echo ❌ 错误：D:\\one_company_desktop\\iqra\\ 不存在或不完整。
    pause
    exit /b 1
)
...
if exist "D:\\hermes-agent-source\\hermes_bootstrap.py" (
    ...
    echo sys.path.insert(0, r'D:\\one_company_desktop\\iqra') >> "D:\\hermes-agent-source\\hermes_bootstrap.py"
...
if exist "D:\\one_company_desktop\\hermes\\hermes_bootstrap.py" (
    ...
    echo sys.path.insert(0, r'D:\\one_company_desktop\\iqra') >> "D:\\one_company_desktop\\hermes\\hermes_bootstrap.py"
    echo    → 已创建 hermes_bootstrap.py
)
...
if exist "%REG_PATH%" (
    findstr /c:"Iqra business tools" "%REG_PATH%" >nul
    if %errorlevel% equ 0 (
        echo    → 已打补铁，跳过
    ) else (
        echo. >> "%REG_PATH%"
        echo # Iqra business tools (auto-synced) >> "%REG_PATH%"
        echo try: >> "%REG_PATH%"
        echo     from iqra.tools.business_tools import register_business_tools >> "%REG_PATH%"
        echo     register_business_tools(registry, data_dir="D:/one_company_desktop/data") >> "%REG_PATH%"
        echo except ImportError as e: >> "%REG_PATH%"
        echo     pass  # iqra not available, skip silently >> "%REG_PATH%"
