...
"一人公司宇宙版"是一套基于 PyQt5 + iqra AI 引擎的全栈桌面管理软件，
...
│   ├── agent/                 # Agent 传输层
│   ├── data/                  # 引擎配置
│   │   ├── iqra_config.json  # 当前活跃 provider 配置
...
│   │   ├── iqra_floating_planet.py # ★ FloatingPlanet 悬浮星球 (1341 行)
...
| `iqra` | AI 引擎根包（路径注入/版本号） |
| `iqra.core` | AgentLoop / ChatEngine / ToolRegistry / SkillLoader / MemoryStore 等 43 个核心模块 |
| `iqra.tools` | 100+ 工具（文件/浏览器/代码/终端/MCP/视觉/TTS 等） |
| `iqra.skills` | 技能注册中心（30+ 领域技能包） |
| `iqra.plugins` | 模型供应商 + 记忆系统 + 图像生成 + Web 搜索 |
| `iqra.agent` | Agent 传输层（transports/） |
| `iqra.data` | 引擎配置持久化 |
| `modules` | 业务模块根包 |
...
# 启动流程：
# 1. 将项目根目录加入 sys.path
# 2. 加载 iqra/data/iqra_config.json
# 3. 初始化 iqra Engine（BackendFactory + AgentBridge）
...
**FloatingPlanet 类位置**：`modules/intelligence/iqra_floating_planet.py`，1341 行。
...
依赖：
- `iqra.core.chat_engine.ChatEngine`
- `iqra.core.tool_registry.ToolRegistry`
- `iqra.core.llm_backend.BaseLLMBackend`
- `iqra.core.agent_loop.AgentLoop`
- `iqra.core.memory_store.MemoryStore`
...
| iqra 引擎 | AI Agent 框架（AgentLoop + ChatEngine + ToolRegistry + SkillLoader） |
| OpenAI 兼容 API | LLM 后端（通过 iqra plugins/model-providers） |
...
| 切换 AI 模型 | 通过 LoginWindow → ModelSetup 或修改 `iqra/data/iqra_config.json` |
| 检查依赖 | `core/deps.py` → `ensure_core_deps()` |
| 运行 iqra 测试 | `cd iqra && python3 test_iqra.py` |
| 查看引擎日志 | `iqra/logs/` 目录 |
...
| 悬浮星球 UI | `modules/intelligence/iqra_floating_planet.py` |
| 数据库 Schema | `core/data.py` |
| 宇宙渲染 | `core/cosmic.py` |
| 星球绘制 | `core/planet_painter.py` |
| 依赖管理 | `core/deps.py` |
| AgentLoop | `iqra/core/agent_loop.py` |
| ChatEngine | `iqra/core/chat_engine.py` |
| ToolRegistry | `iqra/core/tool_registry.py` |
| SkillLoader | `iqra/core/skill_loader.py` |
| MemoryStore | `iqra/core/memory_store.py` |
| LLM 后端 | `iqra/core/llm_backend.py` |
| 引擎配置 | `iqra/data/iqra_config.json` |
