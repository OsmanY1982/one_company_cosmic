...
class AICenterWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        for name, idx in tab_defs:
            ...
            btn.setStyleSheet("""
                QPushButton {
                    padding: 8px 18px; background: #e8e8e8;
        ...
        # 1. 智能总览
        self._tab_stack.addWidget(self._create_overview_tab())
        # 2. 数据分析
        self._tab_stack.addWidget(self._create_analysis_tab())
        # 3. 智能报表
        self._tab_stack.addWidget(self._create_report_tab())
        # 4. 业务洞察
        self._tab_stack.addWidget(self._create_insights_tab())
    ...
    def _execute_query(self):
        ...
        try:
            ...
            if '销售' in query or '金额' in query or 'revenue' in query:
                ...
                cursor.execute("SELECT COUNT(*) FROM product")
                count = cursor.fetchone()[0]
                cursor.execute("SELECT COALESCE(SUM(stock), 0) FROM product")
                ...
                cursor.execute("SELECT name, stock FROM product WHERE stock < 50")
    ...
    def _show_inventory_alert(self):
        ...
        try:
            ...
            cursor.execute("SELECT name, stock FROM product WHERE stock < 50 ORDER BY stock")
    ...
    def _generate_report(self):
        ...
        try:
            ...
            if report_type == "销售报表":
                ...
                cursor.execute("SELECT COUNT(*), COALESCE(SUM(stock), 0) FROM product")
