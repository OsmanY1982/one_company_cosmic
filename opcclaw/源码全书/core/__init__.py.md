...
# 核心引擎
_core = _safe_import('core_engine', ['ToolRegistry', 'OPCclawCoreEngine', 'CoreEngine'],
                     {'OPCclawCoreEngine': 'CoreEngine'})
ToolRegistry = _core.get('ToolRegistry')
CoreEngine = _core.get('CoreEngine') or _core.get('OPCclawCoreEngine')
...
# 协作客户端
_collab = _safe_import('collaboration_client', ['OPCclawHermesClient', 'CollaborationMessage', 'CollaborationResponse'])
OPCclawHermesClient = _collab.get('OPCclawHermesClient')
...
class OPCclawConfig:
    ...
    memory_storage_path: str = "opcclaw/data/memory"
    ...
    # 技能配置
    skills_path: str = "opcclaw/skills"
...
class OPCclawCore:
    ...
    def __init__(self, config: OPCclawConfig = None):
        """
        初始化 OPCclaw 核心
        ...
        Args:
            config: 配置对象
        """
        self.config = config or OPCclawConfig()
        self.version = "2.0"
        ...
        print(f"✓ OPCclaw Core v{self.version} 初始化完成")
    ...
    def _init_modules(self):
        ...
        if self.config.collaboration_enabled:
            self.collaboration = OPCclawHermesClient(
...
def get_core() -> OPCclawCore:
    """获取全局核心实例"""
    global _core_instance
    if _core_instance is None:
        _core_instance = OPCclawCore()
