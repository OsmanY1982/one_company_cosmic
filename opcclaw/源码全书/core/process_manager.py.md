...
> 路径：`core/process_manager.py` | 行数：270
...
```python
"""
OPCclaw Process Manager - 后台进程管理
...
class ProcessManager:
    ...
    def __init__(self):
        ...
        self.log_dir = "D:/one_company_desktop/opcclaw/data/process_logs"
    ...
    def poll(self, process_id: str, limit: int = 200) -> Dict:
        ...
        try:
            output_bytes = b""
            if proc_info.get("stdout_pipe"):
                try:
                    while True:
                        chunk = proc_info["stdout_pipe"].read(4096)
                        if not chunk:
                            break
                        output_bytes += chunk
                except Exception:
