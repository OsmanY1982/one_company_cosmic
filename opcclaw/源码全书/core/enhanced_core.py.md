# `core/enhanced_core.py`

> 路径：`core/enhanced_core.py` | 行数：543
...
# 确保 opcclaw 在路径中
opcclaw_root = Path(__file__).parent.parent
if str(opcclaw_root) not in sys.path:
    sys.path.insert(0, str(opcclaw_root))
...
class OPCclawEnhanced:
    ...
    def __init__(self, skills_dir: str = None, data_dir: str = None):
        ...
        self.data_dir = data_dir or "D:/one_company_desktop/data"
    ...
    def _init_modules(self):
        ...
        # 技能系统
        try:
            from opcclaw.core.skill_system import get_skill_system
        ...
        # Token 优化
        try:
            from opcclaw.core.token_saver import get_token_optimizer
        ...
        # 任务调度
        try:
            from opcclaw.core.task_scheduler import get_scheduler
        ...
        # 会话搜索
        try:
            from opcclaw.core.session_search import get_session_search
        ...
        # 文件补丁
        try:
            from opcclaw.core.patch_engine import get_patch_engine
        ...
        # 子代理委托
        try:
            from opcclaw.core.agent_delegate import get_agent_delegate
        ...
        # TODO 任务清单
        try:
            from opcclaw.core.todo_system import get_todo_system
        ...
        # 模型状态管理器
        try:
            from opcclaw.core.model_status_manager import get_model_status_manager
        ...
        # 后台进程管理
        try:
            from opcclaw.core.process_manager import get_process_manager
        ...
        # 交互确认系统
        try:
            from opcclaw.core.clarify_system import get_clarify_system
        ...
        # Web/GUI 同步桥
        try:
            from opcclaw.core.sync_bridge import get_sync_bridge
...
def get_enhanced(skills_dir: str = None, data_dir: str = None) -> OPCclawEnhanced:
    """获取增强核心单例"""
    global _enhanced
    if _enhanced is None:
        _enhanced = OPCclawEnhanced(skills_dir, data_dir)
    return _enhanced
