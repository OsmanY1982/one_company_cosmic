# `tools/self_monitor.py`

> 路径：`tools/self_monitor.py` | 行数：98
...
class SelfMonitor:
    ...
    def health_check(self) -> dict:
        ...
        # 1. 数据库检查
        import sqlite3
        db_dir = r"D:/one_company_desktop/data"
        dbs = ["orders.db","products.db","customer.db","staff.db","finance.db"]
        for db in dbs:
        ...
        for cf in config_files:
            path = os.path.join(r"D:/one_company_desktop", cf)
            report["checks"]["config_"+cf] = {"exists": os.path.exists(path)}
        
        # 3. 磁盘空间
        disk_free = self._get_disk_space(r"D:")
        report["checks"]["disk_space"] = disk_free
        
        # 4. 技能加载检查
        skills_path = r"D:/one_company_desktop/opcclaw/skills/builtin"
        skill_dirs = [d.name for d in os.scandir(skills_path) if d.is_dir()] if os.path.exists(skills_path) else []
        ...
        if critical_failures > 2:
            ...
            report["recommendations"].append("⚠️ 多个组件异常，请检查配置")
        
        return report
    
    def _get_disk_space(self, drive) -> dict:
        try:
            remaining = os.path.freepath(drive)[0] / (1024**3)
            total = os.path.freepath(drive)[1] / (1024**3)
            return {"total_gb": round(total,1), "free_gb": round(remaining,1), "usage_pct": round((total-remaining)/total*100,1)}
        except:
    ...
    def _tool_count(self):
        try:
            ...
            r = ToolRegistry()
            register_all_tools(r, r"D:/one_company_desktop/data")
            return r.count()
        except:
            return "?"
