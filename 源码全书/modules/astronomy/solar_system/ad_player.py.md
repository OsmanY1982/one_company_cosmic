# `modules/astronomy/solar_system/ad_player.py`

> 路径：`modules/astronomy/solar_system/ad_player.py` | 行数：589
...
from PyQt5.QtCore import Qt, QTimer, QUrl, pyqtSignal, QRectF
...
class CountdownAdPage(QWidget):
    ...
    def _tick(self):
        self._countdown -= 1
        self._countdown_widget.set_progress(
            1 - self._countdown / self.COUNTDOWN_SECONDS
        )
...
class _CountdownRing(QWidget):
    ...
    def paintEvent(self, event):
        ...
        p.setFont(font)
        secs = max(0, int((1 - self._progress) * CountdownAdPage.COUNTDOWN_SECONDS))
        p.drawText(QRectF(cx - r, cy - r, r * 2, r * 2),
                   Qt.AlignCenter, str(secs))
...
class AdPlayerWidget(QWidget):
    ...
    def _play_countdown(self) -> bool:
        ...
        self._ad_duration = CountdownAdPage.COUNTDOWN_SECONDS
        self._elapsed = 0
        self._completed = False
