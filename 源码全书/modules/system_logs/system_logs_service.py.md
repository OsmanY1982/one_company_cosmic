# `modules/system_logs/system_logs_service.py`

> 路径：`modules/system_logs/system_logs_service.py` | 行数：181
...
def init_error_logs_db():
    """创建错误日志表（如不存在）"""
    ensure_tables("system_logs.db", [_ERROR_LOGS_DDL, _SYNC_LOGS_DDL])
...
def get_operation_logs(op_type: str = "全部", date_from: str = None,
    ...
    try:
        ...
        return []
    query = "SELECT id, created_at, username, action, module, detail FROM operation_logs WHERE 1=1"
    params = []
    if op_type != "全部":
