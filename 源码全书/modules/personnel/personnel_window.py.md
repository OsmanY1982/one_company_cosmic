...
> 路径：`modules/personnel/personnel_window.py` | 行数：738
...
DAO 层保留，UI 改为星球环绕导航
"""
import sqlite3, csv, os, json, math
...
def _get_conn(db_name: str):
    conn = sqlite3.connect(_get_db_path(db_name))
...
def member_stats():
    ...
    conn.close(); return {"total": total, "levels": levels}
...
class PersonnelWindow(QMainWindow):
    ...
    def _build_ui(self):
        ...
        title = QLabel("居住环")
        title.setStyleSheet(
            "color: #ffccaa; font-size: 24px; font-weight: 800; "
            "letter-spacing: 8px; background: transparent;"
        )
        hl.addWidget(title, alignment=Qt.AlignCenter)

        subtitle = QLabel("CREW QUARTERS · 点击星球进入对应模块")
        subtitle.setStyleSheet(
            "color: #997766; font-size: 11px; letter-spacing: 3px; background: transparent;"
        )
