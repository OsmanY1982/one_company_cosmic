# `modules/auth/activation_gate.py`

> 路径：`modules/auth/activation_gate.py` | 行数：307
...
class ActivationGateWindow(QMainWindow):
    ...
    def _do_pay_activate(self):
        self._do_activate(self.pay_code_input.text().strip())

    def _do_direct_activate(self):
    ...
    def _go_next(self):
        self.hide()
        def _show():
            from modules.login.select_mode_window import SelectModeWindow
            sw = SelectModeWindow()
            sw.show()
            sw.activateWindow()
