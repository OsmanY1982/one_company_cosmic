...
class AIChatWindow(QWidget):
    ...
    def __init__(self, parent=None, iqra_engine=None, floating_mode=False, voice=None, embedded=False):
    ...
    def _build_ui(self):
        ...
        if self._bridge is not None:
            ...
            status_text = f"AgentBridge: {prov.get('name', 'Iqra')} / {prov.get('model', self._current_model)}"
            status_color = "#44cc88"
        else:
    ...
    def _refresh_model_list(self):
        ...
        if self._bridge and hasattr(self._bridge, "get_provider_info"):
            ...
            self.lbl_status.setText(
                f"AgentBridge: {prov.get('name', 'Iqra')} / {prov.get('model', self._current_model)}"
            )
            self.lbl_status.setStyleSheet("color: #44cc88; font-size: 11px; background: transparent;")
