...
> 路径：`modules/intelligence/opcclaw_floating_planet.py` | 行数：1714
...
class FloatingPlanet(QWidget):
    ...
    def _query_ai(self, text: str):
        ...
        # TTS 朗读回复 — 统一走 VoiceInterface 通道，避免竞态
    ...
    def _terminate_speak(self):
        """终止所有语音播报（VoiceInterface 合成器 + 旧 say 进程）"""
        if self._voice:
...
# ═══════════ AI 对话弹窗（内嵌） ═══════════
    ...
    PERSONALITY_VOICES: dict = {}  # 在 __init__ 末尾 _build_personality_voices() 中构建

    @staticmethod
    def _build_personality_voices() -> dict:
        """构建每种形态的个性化语音配置：(voice, rate, [quotes...])"""
        # 星球形态 — 宇宙天体的人格化语录
        pv = {
            # ──── 星球形态 ────
            "classic":      ("Flo", 180, [
    ...
    def _speak_shape(self, name: str, key: str):
        """形态切换时的个性化语音播报 — 走 VoiceInterface 统一通道，避免竞态"""
        if not self._voice:
            return
