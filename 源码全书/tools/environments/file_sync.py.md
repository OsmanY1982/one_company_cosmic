...
> 路径：`tools/environments/file_sync.py` | 行数：522
...
# ═══════════════════════════════════════════
#  GUI Panel wrapper for Cloud Sync Tab
# ═══════════════════════════════════════════
...
class FileSyncPanel(QWidget):
    """File Sync GUI panel — embedded in cloud_window.py Tab 2.

    Displays sync status, provides manual sync trigger, and shows a
    scrolling sync log.  Thin UI wrapper; actual sync logic lives in
    FileSyncManager when backends are active.
    """
    ...
    def _build(self):
        ...
        self._status_label = QLabel("就绪 — 等待同步")
        self._status_label.setStyleSheet("color: #44cc88; font-size: 13px;")
        s_layout.addWidget(self._status_label)

        info = QLabel("文件同步管理器跟踪本地文件变更，并与远程环境保持同步。\n"
                       "Docker / Singularity 使用绑定挂载，不需要此模块。")
        info.setWordWrap(True)
        info.setStyleSheet("color: #778899; font-size: 11px;")
        s_layout.addWidget(info)
        layout.addWidget(sg)

        # Manual sync
        btn_row = QHBoxLayout()
        sync_btn = QPushButton("手动同步")
        ...
        """)
        sync_btn.clicked.connect(self._manual_sync)
        ...
        self._append_log("FileSync 面板已就绪")
    ...
    def _manual_sync(self):
        ...
        self._status_label.setStyleSheet("color: #f0c040; font-size: 13px;")
        self._append_log("手动同步已触发（当前无活跃后端，跳过）")
        # Simulate brief delay then reset status
        QTimer.singleShot(500, lambda: self._status_label.setText("就绪 — 等待同步"))
        QTimer.singleShot(500, lambda: self._status_label.setStyleSheet("color: #44cc88; font-size: 13px;"))
