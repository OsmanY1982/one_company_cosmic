...
> 路径：`core/shapes/crystal_alien.py` | 行数：222
...
def paint(painter: QPainter, center: QPointF, radius: float,
    ...
    # ── 环绕粒子光环（主题色匹配）──
    aura_rng = random.Random(int(anim_t * 280) % 100000 + 72896)
    p.setPen(Qt.NoPen)
    for _ in range(22):
        ...
        ag.setColorAt(0.0, QColor(160, 200, 255, a_alpha))
        ag.setColorAt(0.5, QColor(160//2, 200//2, 255, a_alpha // 2))
...
# ── 悬停增强（主题色脉冲光晕 + 呼吸轮廓）──
    if hovered:
        ...
        for i in range(3):
            ...
            ig.setColorAt(0.78, QColor(160, 200, 255, ga // 2))
            ig.setColorAt(0.90, QColor(160, 200, 255, ga))
            ig.setColorAt(0.97, QColor(160//2, 200//2, 255, ga // 3))
        ...
        for i in range(3):
            ...
            og.setColorAt(0.88, QColor(160, 200, 255, ga // 2))
            og.setColorAt(0.96, QColor(160//2, 200//2, 255, ga // 3))
        ...
        rpen = QPen(QColor(160, 200, 255, int(220 * hp * br)), 2.5 + 1.0 * br)
