# `core/shapes/jellyfish_alien.py`

> 路径：`core/shapes/jellyfish_alien.py` | 行数：179
...
def paint(painter: QPainter, center: QPointF, radius: float,
    ...
    # ── 悬停高亮环 ──
    if hovered:
        h_pulse = 0.7 + 0.3 * abs(math.sin(anim_t * 3.5))
        pen = QPen(QColor(80, 240, 200, int(200 * h_pulse)), 2.0 * s)
        p.setPen(pen); p.setBrush(Qt.NoBrush)
