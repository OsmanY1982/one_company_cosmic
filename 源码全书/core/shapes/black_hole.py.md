...
> 路径：`core/shapes/black_hole.py` | 行数：135
...
def paint(painter: QPainter, center: QPointF, radius: float,
    ...
    # ── 悬停 ──
    if hovered:
        p_hp = 0.7 + 0.3 * abs(math.sin(anim_t * 3.5))
        pen = QPen(QColor(180, 120, 255, int(200 * p_hp)), 2.0)
        p.setPen(pen); p.setBrush(Qt.NoBrush)
        p.drawEllipse(center, radius + 2, radius + 2)
        for i in range(2):
            outer_r = radius + 8 + i * 8
            og = QRadialGradient(center, outer_r)
            ga = int((45 - i * 14) * p_hp)
            og.setColorAt(0.7, QColor(255, 255, 255, 0))
            og.setColorAt(0.88, QColor(100, 50, 200, ga // 2))
            og.setColorAt(0.97, QColor(140, 80, 240, ga))
