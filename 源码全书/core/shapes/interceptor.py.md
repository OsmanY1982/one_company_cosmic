...
> 路径：`core/shapes/interceptor.py` | 行数：501
...
"""
截击舰形态 — 悬浮球变形
真实感设计：多层金属矛形船体 + 装甲板线 + 散热格栅 + 四门速射炮 + 三层引擎尾焰
适配悬浮球 ~40px 半径绘制区
"""
import math
import random
...
def paint(p: QPainter, center: QPointF, radius: float, anim_t: float,
    ...
    _paint_side_blades(p, cx, left, top, w, h, size, alpha)
...
def _paint_main_hull(p, cx, left, top, w, h, size, alpha):
    ...
    shadow_path.cubicTo(cx + w * 0.03, nose_y + h * 0.03,
                        cx + mid_hw * 0.85, top + h * 0.35,
                        cx + mid_hw * 0.65, top + h * 0.55)
    shadow_path.lineTo(cx + tail_hw * 0.85, body_bot_y + h * 0.02)
    shadow_path.lineTo(cx - tail_hw * 0.85, body_bot_y + h * 0.02)
    shadow_path.cubicTo(cx - mid_hw * 0.65, top + h * 0.55,
                        cx - mid_hw * 0.85, top + h * 0.35,
                        cx - w * 0.03, nose_y + h * 0.03)
    ...
    p.setPen(Qt.NoPen)
    p.drawRect(QRectF(cx - tail_hw * 0.85, body_bot_y, tail_hw * 1.7, h * 0.04))


def _paint_side_blades(p, cx, left, top, w, h, size, alpha):
    """侧翼刀片 — 截击舰特有的锐利侧翼"""
    for sign in [-1, 1]:
        blade_path = QPainterPath()
        b_top_x = cx + sign * w * 0.10
        b_top_y = top + h * 0.22
        b_mid_x = cx + sign * w * 0.28
        b_mid_y = top + h * 0.38
        b_bot_x = cx + sign * w * 0.15
        b_bot_y = top + h * 0.50

        blade_path.moveTo(b_top_x, b_top_y)
        blade_path.cubicTo(b_top_x + sign * w * 0.06, b_top_y + h * 0.04,
                           b_mid_x, b_mid_y - h * 0.02,
                           b_mid_x, b_mid_y)
        blade_path.lineTo(b_bot_x, b_bot_y)
        blade_path.cubicTo(cx + sign * w * 0.06, b_bot_y,
                           cx + sign * w * 0.03, b_top_y + h * 0.08,
                           b_top_x, b_top_y)
        blade_path.closeSubpath()

        blade_grad = QLinearGradient(cx, b_top_y, cx, b_bot_y)
        blade_grad.setColorAt(0.0, QColor(0x2a, 0x2d, 0x33, int(230 * alpha)))
        blade_grad.setColorAt(0.5, QColor(0x48, 0x4c, 0x53, int(225 * alpha)))
        blade_grad.setColorAt(1.0, QColor(0x20, 0x22, 0x28, int(220 * alpha)))
        p.setBrush(blade_grad)
        p.setPen(QPen(QColor(0x1a, 0x1d, 0x22, int(150 * alpha)), 0.6))
        p.drawPath(blade_path)
...
def _paint_superstructure(p, cx, left, top, w, h, size, alpha):
    ...
    path.cubicTo(cx + spine_hw_bot * 0.5, top + h * 0.12,
                 cx + spine_hw_bot, top + h * 0.15,
                 cx + spine_hw_bot, spine_bot)
    path.lineTo(cx - spine_hw_bot, spine_bot)
    path.cubicTo(cx - spine_hw_bot, top + h * 0.15,
                 cx - spine_hw_bot * 0.5, top + h * 0.12,
                 cx - spine_hw_top, spine_top + h * 0.015)
    path.closeSubpath()
    ...
    bridge_path.addRoundedRect(QRectF(bridge_x, bridge_y, bridge_w, bridge_h), 2, 2)
    ...
    # 舰桥高光
    hl = QRectF(bridge_x + 1, bridge_y + 1, bridge_w - 2, bridge_h * 0.35)
...
def _paint_armor_panels(p, cx, left, top, w, h, size, alpha):
    ...
    base_y = top + h * 0.28
    for i in range(4):
        ly = base_y + i * h * 0.06 * (1 + i * 0.25)
        frac = (ly - top) / h
        hw = w * (0.18 - frac * 0.12)
        p.drawLine(QPointF(cx - hw, ly), QPointF(cx + hw, ly))

    # V形装甲线（2条）
    v_center_y = top + h * 0.54
    for sign in [-1, 1]:
        vx1 = cx + sign * w * 0.10
        vx2 = cx + sign * w * 0.03
        vy1 = v_center_y
        vy2 = v_center_y + h * 0.07
        p.drawLine(QPointF(vx1, vy1), QPointF(vx2, vy2))

    # 纵向中线
    p.drawLine(QPointF(cx, top + h * 0.12), QPointF(cx, top + h * 0.24))

    # 尾段横线
    p.drawLine(QPointF(cx - w * 0.07, top + h * 0.62),
               QPointF(cx + w * 0.07, top + h * 0.62))
...
def _paint_rivets(p, cx, left, top, w, h, size, alpha):
    ...
    rivet_y = [0.28, 0.34, 0.40, 0.46, 0.54, 0.61]
    for ryf in rivet_y:
        ry = top + h * ryf
        frac = (ry - top) / h
        hw = w * (0.18 - frac * 0.12)
...
def _paint_antennas(p, cx, left, top, w, h, size, alpha):
    ...
    p.setPen(Qt.NoPen)
    p.drawEllipse(QPointF(ant_x, ant_tip), size * 0.016, size * 0.016)

    # 副天线（左右不对称高度）
    for sign, height_ratio in [(-1, 0.85), (1, 0.60)]:
        sax = cx + sign * w * 0.06
        sabase = top + h * 0.08
        satip = top - h * 0.06 * height_ratio
...
def _paint_engine_nacelles(p, cx, left, top, w, h, size, anim_t, alpha):
    """引擎舱结构环 — 3 个引擎（中1+侧2，三角形布局）"""
    engine_offsets = [(-w * 0.10, top + h * 0.76),
                      (0, top + h * 0.80),
                      (w * 0.10, top + h * 0.76)]
    pulse = 0.7 + 0.3 * abs(math.sin(anim_t * 4.0))

    for offset_x, ey in engine_offsets:
        ex = cx + offset_x

        nacelle_grad = QRadialGradient(ex, ey, size * 0.075)
        ...
        p.drawEllipse(QPointF(ex, ey), size * 0.075, size * 0.05)

        for ring_i, ring_scale in enumerate([0.7, 0.40]):
            rr_x = size * 0.075 * ring_scale
            rr_y = size * 0.05 * ring_scale
            p.setPen(QPen(QColor(0x7a, 0x7e, 0x85, int((80 - ring_i * 20) * pulse * alpha)), 0.4))
            p.setBrush(Qt.NoBrush)
            p.drawEllipse(QPointF(ex, ey), rr_x, rr_y)

        core_glow = QRadialGradient(ex, ey, size * 0.022)
        ...
        p.setPen(Qt.NoPen)
        p.drawEllipse(QPointF(ex, ey), size * 0.022, size * 0.015)


def _paint_engine_flames(p, cx, left, top, w, h, size, anim_t, alpha):
    """引擎尾焰 — 三层叠加，3 个引擎"""
    pulse = 0.55 + 0.45 * abs(math.sin(anim_t * 8.0))
    engine_positions = [(-w * 0.10, top + h * 0.76),
                        (0, top + h * 0.80),
                        (w * 0.10, top + h * 0.76)]

    random.seed(42)
    for idx, (offset_x, ey) in enumerate(engine_positions):
        ex = cx + offset_x
        length_factor = 1.0 + random.uniform(-0.20, 0.20)

        flame_base_h = size * 0.32 * pulse * length_factor
        ...
        # 内核
        core_grad = QLinearGradient(ex, ey, ex, ey + flame_base_h * 0.28)
...
def _paint_weapons(p, cx, left, top, w, h, size, anim_t, alpha):
    """武器系统 — 4 门速射炮，两两对称分布"""
    charge = 0.4 + 0.6 * abs(math.sin(anim_t * 6.0))

    gun_configs = [
        (cx - w * 0.25, top + h * 0.38, size * 0.035, size * 0.09),  # 左上
        (cx + w * 0.25, top + h * 0.38, size * 0.035, size * 0.09),  # 右上
        (cx - w * 0.16, top + h * 0.50, size * 0.030, size * 0.08),  # 左下
        (cx + w * 0.16, top + h * 0.50, size * 0.030, size * 0.08),  # 右下
    ]

    for gun_x, gun_y, gun_w, gun_h in gun_configs:
        ...
        muzzle_glow.setColorAt(0.0, QColor(200, 200, 255, int(200 * charge * alpha)))
        muzzle_glow.setColorAt(0.6, QColor(100, 150, 255, int(80 * charge * alpha)))
