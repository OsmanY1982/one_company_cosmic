# `core/notification_cron.py`

> 路径：`core/notification_cron.py` | 行数：181
...
def check_member_expiry():
    ...
    try:
        # 检查VIP会员（7天内到期）
        conn = sqlite3.connect(f"{DATA_DIR}/member.db")
        cursor = conn.cursor()
        today = datetime.now()
...
def check_pending_orders():
    ...
    alerts = []
    try:
        conn = sqlite3.connect(f"{DATA_DIR}/order.db")
        cursor = conn.cursor()
...
def check_low_stock():
    ...
    alerts = []
    try:
        conn = sqlite3.connect(f"{DATA_DIR}/orders.db")
        cursor = conn.cursor()
