# `iqra/tools/delegate_tool.py`

> 路径：`iqra/tools/delegate_tool.py` | 行数：2767
...
def _build_child_agent(
    ...
    try:
        ...
        if delegation_effort:
            from iqra_constants import parse_reasoning_effort
...
def delegate_task(
    ...
    try:
        from iqra_cli.plugins import invoke_hook as _invoke_hook
    except Exception:
        _invoke_hook = None
...
def _load_config() -> dict:
    ...
    to the persistent config (iqra_cli/config.py load_config()) so that
