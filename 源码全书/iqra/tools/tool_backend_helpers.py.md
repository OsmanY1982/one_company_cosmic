# `iqra/tools/tool_backend_helpers.py`

> 路径：`iqra/tools/tool_backend_helpers.py` | 行数：144
...
def managed_nous_tools_enabled() -> bool:
    ...
    """
    try:
        from iqra_cli.auth import get_nous_auth_status

        status = get_nous_auth_status()
        if not status.get("logged_in"):
            return False

        from iqra_cli.models import check_nous_free_tier
...
def fal_key_is_configured() -> bool:
    ...
    ``iqra_cli.config.get_env_value`` when available) so tool-side
    ...
    if value is None:
        ...
        try:
            from iqra_cli.config import get_env_value
