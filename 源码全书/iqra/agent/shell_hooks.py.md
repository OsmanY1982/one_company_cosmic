# `iqra/agent/shell_hooks.py`

> 路径：`iqra/agent/shell_hooks.py` | 行数：836
...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`iqra_cli.plugins.invoke_hook` and its aggregators.  Python
...
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``iqra_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
from iqra_constants import get_hermes_home
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``iqra_cli.config.load_config``
    ...
    from iqra_cli.plugins import get_plugin_manager
...
def _parse_hooks_block(hooks_cfg: Any) -> List[ShellHookSpec]:
    ...
    """
    from iqra_cli.plugins import VALID_HOOKS
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    ...
    :func:`iqra_cli.plugins.get_pre_tool_call_block_message`.  This is
...
def run_once(
    ...
    ``kwargs`` is the same dict that :func:`iqra_cli.plugins.invoke_hook`
