# `iqra/agent/credential_pool.py`

> 路径：`iqra/agent/credential_pool.py` | 行数：1603
...
from iqra_cli.config import get_env_value, load_env
import iqra_cli.auth as auth_mod
...
def _iter_custom_providers(config: Optional[dict] = None):
    ...
    if not isinstance(custom_providers, list):
        ...
        try:
            from iqra_cli.config import get_compatible_custom_providers
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from iqra_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
    ...
    if provider == "anthropic":
        ...
        try:
            from iqra_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
        ...
        try:
            from iqra_cli.auth import resolve_qwen_runtime_credentials
        ...
        try:
            from iqra_cli.auth import get_provider_auth_state
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    try:
        from iqra_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
