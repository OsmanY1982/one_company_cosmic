# `iqra/agent/auxiliary_client.py`

> 路径：`iqra/agent/auxiliary_client.py` | 行数：4520
...
from iqra_cli.config import get_hermes_home
from iqra_constants import OPENROUTER_BASE_URL
...
from iqra_cli import __version__ as _HERMES_VERSION
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``iqra_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _resolve_nous_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    ...
    """
    try:
        from iqra_cli.auth import resolve_nous_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    ...
    try:
        from iqra_cli.auth import _read_codex_tokens
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from iqra_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from iqra_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from iqra_cli.models import copilot_default_headers
...
def _try_nous(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    model = _NOUS_MODEL
    try:
        from iqra_cli.models import get_nous_recommended_aux_model
...
def _resolve_custom_runtime() -> Tuple[Optional[str], Optional[str], Optional[str]]:
    ...
    """
    try:
        from iqra_cli.runtime_provider import resolve_runtime_provider
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        if normalized == "openai-codex":
            from iqra_cli.auth import resolve_codex_runtime_credentials
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from iqra_cli.copilot_auth import copilot_request_headers
...
def _normalize_resolved_model(model_name: Optional[str], provider: str) -> Optional[str]:
    ...
    if not model_name:
        return model_name
    try:
        from iqra_cli.model_normalize import normalize_model_for_provider
...
def resolve_provider_client(
    ...
    try:
        from iqra_cli.runtime_provider import _get_named_custom_provider
    ...
    try:
        ...
        logger.debug("iqra_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from iqra_cli.models import _should_use_copilot_responses_api
