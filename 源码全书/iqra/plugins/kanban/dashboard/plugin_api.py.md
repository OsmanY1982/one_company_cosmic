...
``iqra_cli.kanban_db`` or a direct SQL query. Writes use the same code
...
bridge in ``iqra_cli/web_server.py``. If you run the dashboard with
...
from iqra_cli import kanban_db
...
def _check_ws_token(provided: Optional[str]) -> bool:
    ...
    if not provided:
        return False
    try:
        from iqra_cli import web_server as _ws
    except Exception:
...
def _compute_task_diagnostics(
    ...
    Uses ``iqra_cli.kanban_diagnostics`` — see that module for the
    rule definitions.
    """
    from iqra_cli import kanban_diagnostics as kd
...
def _warnings_summary_from_diagnostics(
    ...
    if not diagnostics:
        return None
    from iqra_cli.kanban_diagnostics import SEVERITY_ORDER
...
def create_task(payload: CreateTaskBody, board: Optional[str] = Query(None)):
    ...
    try:
        ...
        if task and task.status == "ready" and task.assignee:
            try:
                from iqra_cli.kanban import _check_dispatcher_presence
...
# spawn failures, stuck-blocked). See iqra_cli.kanban_diagnostics for
# the rule engine.
# ---------------------------------------------------------------------------
...
def specify_task_endpoint(
    ...
    try:
        ...
        from iqra_cli import kanban_specify  # noqa: WPS433 (intentional)
...
async def stream_events(ws: WebSocket):
    ...
    # authenticates in iqra_cli/web_server.py.
