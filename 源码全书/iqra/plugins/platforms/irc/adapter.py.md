# `iqra/plugins/platforms/irc/adapter.py`

> 路径：`iqra/plugins/platforms/irc/adapter.py` | 行数：969
...
def interactive_setup() -> None:
    ...
    Lazy-imports ``iqra_cli.setup`` helpers so the plugin stays importable
    in non-CLI contexts (gateway runtime, tests).
    """
    from iqra_cli.setup import (
        prompt,
        prompt_yes_no,
        save_env_value,
        get_env_value,
