# `opcclaw/hermes_constants.py`

> 路径：`opcclaw/hermes_constants.py` | 行数：95
...
#!/usr/bin/env python3
"""
Hermes Constants — OPCclaw 兼容层

此模块为缺失的 hermes_constants 模块提供存根实现。
原 Hermes 项目中的常量/路径函数在此适配为 OPCclaw 环境。
...
def is_termux() -> bool:
    """检测是否为 Termux 环境。OPCclaw 不在 Termux 运行。"""
    return False
