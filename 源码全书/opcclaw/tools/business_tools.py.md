# `opcclaw/tools/business_tools.py`

> 路径：`opcclaw/tools/business_tools.py` | 行数：489
...
def _connect(db_dir: str, db_name: str) -> sqlite3.Connection:
    path = os.path.join(db_dir, db_name)
    c = sqlite3.connect(path)
    c.row_factory = sqlite3.Row
    c.text_factory = lambda x: str(x, 'utf-8', 'replace')
    return c
...
def query_products(data_dir: str, keyword: str = "") -> dict:
    ...
    db = _connect(data_dir, "products.db")
    try:
        if keyword:
            rows = _dict_rows(db.execute(
                "SELECT name,price,stock,category,description FROM products WHERE name LIKE ? LIMIT 50",
                (f"%{keyword}%",)
            ))
        else:
            rows = _dict_rows(db.execute(
                "SELECT name,price,stock,category,description FROM products LIMIT 50"
            ))
...
def query_orders(data_dir: str, month: str = "") -> dict:
    ...
    db = _connect(data_dir, "orders.db")
    try:
        if month:
            rows = _dict_rows(db.execute(
...
def get_summary_stats(data_dir: str) -> dict:
    ...
    # 订单
    if os.path.exists(os.path.join(data_dir, "orders.db")):
        db = _connect(data_dir, "orders.db")
        try:
    ...
    # 产品
    if os.path.exists(os.path.join(data_dir, "products.db")):
        db = _connect(data_dir, "products.db")
        try:
            stats["产品数"] = db.execute("SELECT COUNT(*) as c FROM products").fetchone()["c"]
        finally:
            db.close()
...
def register_all_tools(registry, data_dir: str):
    ...
    Usage:
        from opcclaw.tools.main_tools import register_all_tools
    ...
    # 注册高级分析工具
    try:
        from opcclaw.tools.analysis_tools import register_analysis_tools
    ...
    # 注册自动化工具
    try:
        from opcclaw.tools.automation_tools import register_automation_tools
    ...
    # 注册营销推广工具
    try:
        from opcclaw.tools.marketing_tools import register_marketing_tools
    ...
    # 注册HR管理工具
    try:
        from opcclaw.tools.hr_tools import register_hr_tools
    ...
    # 注册采购供应链工具
    try:
        from opcclaw.tools.procurement_tools import register_procurement_tools
    ...
    # 注册文档处理工具
    try:
        from opcclaw.tools.doc_tools import register_doc_tools
    ...
    # 注册日程管理工具
    try:
        from opcclaw.tools.scheduling_tools import register_scheduling_tools
    ...
    # 注册AI自检工具
    try:
        from opcclaw.tools.self_monitor import register_monitor_tools
    ...
    # 注册模板库工具
    try:
        from opcclaw.tools.template_tools import register_template_tools
    ...
    # 注册财务分析工具
    try:
        from opcclaw.tools.finance_analysis import register_finance_tools
    ...
    # 注册项目管理工具
    try:
        from opcclaw.tools.project_management import register_project_management_tools
    ...
    # 注册网络搜索工具
    try:
        from opcclaw.tools.web_search_tools import register_web_search_tools
    ...
    # 注册智能报告工具
    try:
        from opcclaw.tools.smart_report_tools import register_smart_report_tools
    ...
    # 注册数据导入工具
    try:
        from opcclaw.tools.data_import_tools import register_data_import_tools
    ...
    # 注册智能提醒工具
    try:
        from opcclaw.tools.alert_tools import register_alert_tools
    ...
    # 注册库存管理工具
    try:
        from opcclaw.tools.inventory_tools import register_inventory_tools
    ...
    # 注册 CRM 增强工具
    try:
        from opcclaw.tools.crm_tools import register_crm_tools
    ...
    # 注册报表导出工具
    try:
        from opcclaw.tools.export_tools import register_export_tools
