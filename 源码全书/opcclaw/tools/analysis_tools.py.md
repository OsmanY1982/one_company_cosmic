# `opcclaw/tools/analysis_tools.py`

> 路径：`opcclaw/tools/analysis_tools.py` | 行数：449
...
class AnalysisTools:
    ...
    def predict_inventory_needs(self, product_id: str = None, days_ahead: int = 30) -> Dict[str, Any]:
        ...
        try:
            # 获取所有产品
            if product_id:
                cursor = prod_db.execute("SELECT * FROM products WHERE id=?", (product_id,))
            else:
                cursor = prod_db.execute("SELECT * FROM products")
