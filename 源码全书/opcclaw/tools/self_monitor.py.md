# `opcclaw/tools/self_monitor.py`

> 路径：`opcclaw/tools/self_monitor.py` | 行数：114
...
# 项目根目录自动检测（tools/self_monitor.py → opcclaw/ → one_company_desktop/）
_PROJECT_ROOT = Path(__file__).parent.parent  # opcclaw/
...
class SelfMonitor:
    ...
    def _tool_count(self):
        try:
            ...
            r = ToolRegistry()
            register_all_tools(r, self.data_dir)
            return r.count()
        except:
            return "?"
