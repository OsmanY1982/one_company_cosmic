# `opcclaw/web_ui/workflow/compiler.py`

> 路径：`opcclaw/web_ui/workflow/compiler.py` | 行数：414
    ...
    for nid in order:
        ...
        if ntype == NODE_INPUT:
            ...
            if config.get("use_rag"):
                imports.add("from opcclaw.core.rag_context import RAGContextInjector")
...
def _wrap_script(body: str, imports: List[str], name: str) -> str:
    ...
    header = f'''"""
工作流: {name}
由 opcclaw Workflow Compiler 自动生成
"""
