# `opcclaw/core/opcclaw_logging.py`

> 路径：`opcclaw/core/opcclaw_logging.py` | 行数：281
...
def _create_logger(name: str) -> logging.Logger:
    ...
    file_handler = RotatingFileHandler(
        os.path.join(LOG_DIR, f"opcclaw_{today}.log"),
    ...
    json_handler = RotatingFileHandler(
        os.path.join(LOG_DIR, f"opcclaw_{today}.jsonl"),
        maxBytes=MAX_LOG_SIZE,
        backupCount=3,
        encoding="utf-8",
    )
...
logger = _create_logger("opcclaw")


def get_logger(name: str) -> logging.Logger:
    """获取子模块 logger"""
    return _create_logger(f"opcclaw.{name}")
