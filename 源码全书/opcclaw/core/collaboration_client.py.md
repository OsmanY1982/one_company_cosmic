# `opcclaw/core/collaboration_client.py`

> 路径：`opcclaw/core/collaboration_client.py` | 行数：495
...
```python
"""
OPCclaw × Hermes Collaboration Client
实现 OPCclaw 与 Hermes 的协作协议
"""
...
class OPCclawHermesClient:
    """
    OPCclaw-Hermes 协作客户端
    ...
    def _socket_listener(self):
        """Socket 监听循环"""
        socket_path = "/tmp/opcclaw_hermes.sock"
    ...
    def _send_via_socket(self, message: CollaborationMessage):
        """通过 Socket 发送消息"""
        socket_path = "/tmp/hermes_opcclaw.sock"
...
if __name__ == "__main__":
    print("OPCclaw × Hermes Collaboration Client")
    print("=" * 50)
    
    # 创建客户端（不使用 Socket，仅 HTTP）
    client = OPCclawHermesClient(use_local_socket=False)
